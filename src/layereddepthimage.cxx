...
cgv::math::mat<int> LayeredDepthImage::get_density() const {
  ...
  for (int i = 0; i < m_layered_points.size(); ++i) {
    ...
    density(y_pos, x_pos) = m_layered_points[i].point_count();
    // Emulates division of 1D index onto 2D width and height
    y_pos = x_pos == width - 1 ? y_pos + 1 : y_pos;
...
void LayeredDepthImage::warp_reference_into(const LayeredDepthImage &ldi) {
  ...
  for (size_t i = 0; i < other_points.size(); i++) {
    auto const morphed_point = morphing_equation(
        other_perspective, this_perspective, other_origin, other_points[i]);

    // If the morphed point is outside of our target camera, skip further
    // computations
    auto const resolution = get_camera().get_resolution();
    if (morphed_point.x() < 0.f || morphed_point.y() < 0.f ||
        morphed_point.x() >= resolution.first ||
        morphed_point.y() >= resolution.second || isnan(morphed_point.x()) ||
        isnan(morphed_point.y()))
      continue;
    else {
      point_t const new_point = {other_color[i], morphed_point.z()};
...
void LayeredDepthImage::add_global_points(const std::vector<float> &points,
  ...
  for (size_t i = 0; i < (points.size() / 3); i++) {
    ...
    vec4 point =
        ...
        vec3(points.at(3 * i), points.at(3 * i + 1), points.at(3 * i + 2))
            .lift();

    vec3 const p(3, point / point.w());
    // Clip points outside of view frustum
    if (!is_unit(p.x()) || !is_unit(p.y()) || !is_unit(p.z())) continue;

    rgb const c(colors.at(3 * i), colors.at(3 * i + 1), colors.at(3 * i + 2));
    point_t new_point;
    new_point.depth = p.z();
    new_point.color = c;
...
std::vector<vec3> LayeredDepthImage::position_data() const {
  ...
  buffer.reserve(m_camera.get_resolution().first *
                 m_camera.get_resolution().second);

  auto const to_world = cgv::math::inv(
      m_camera.get_sensor() * m_camera.get_projection() * m_camera.get_view());

  size_t x_pos{0};
  size_t y_pos{0};
  // Simplify condition check with - 1
  size_t const width = m_camera.get_resolution().first - 1;
  auto const ldi_size = m_layered_points.size();

  for (size_t i = 0; i < ldi_size; ++i) {
    // auto const location = to_coord(i);
    // To avoid costly general computation of 2D position involving div and mod
    // use progressive addition at the end of loop

    for (auto const &point : m_layered_points[i].underlying_data()) {
      vec4 window_pos(x_pos, y_pos, point.depth, 1);
      auto const gpos = to_world * window_pos;

      buffer.emplace_back(gpos.x() / gpos.w(), gpos.y() / gpos.w(),
                          gpos.z() / gpos.w());
    }

    // Emulates division of 1D index onto 2D width and height
    y_pos = x_pos == width ? y_pos + 1 : y_pos;
    x_pos = x_pos == width ? 0 : x_pos + 1;
  }
...
std::vector<rgb> LayeredDepthImage::color_data() const {
  ...
  buffer.reserve(m_layered_points.size());
...
PinholeCameraModel LayeredDepthImage::get_camera() const { return m_camera; }

size_t LayeredDepthImage::point_count() const { return m_point_count; }
...
size_t LayeredDepthImage::to_index(const int x, const int y) const {
  ...
  const size_t index =
      static_cast<size_t>(y) * m_camera.get_resolution().first +
      static_cast<size_t>(x);
  return index;
}

std::pair<size_t, size_t> LayeredDepthImage::to_coord(
    const size_t index) const {
  auto const resolution = m_camera.get_resolution();
  assert(index <= resolution.first * resolution.second);

  size_t const second = index / resolution.first;
  // size_t const first = index % resolution.first;
  // -> Avoid costly modulo operation
  size_t const first = index - second * resolution.first;

  std::pair<size_t, size_t> const coord{first, second};
...
vec3 morphing_equation(const mat4 &perspective_source,
  ...
  auto morphed_position =
      delta * target_per_inv * to_source_center.lift() +
      target_per_inv * perspective_source * source_position.lift();

  // perspective divide
  morphed_position /= morphed_position.w();

  return vec3(morphed_position.x(), morphed_position.y(), morphed_position.z());
}
