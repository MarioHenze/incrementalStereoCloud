...
bool PointCloudSource::unprocessed_present() const {
  if (m_pending_queries.empty()) return false;

  auto const first_uncomplete =
      std::find_if(m_pending_queries.cbegin(), m_pending_queries.cend(),
...
void PointCloudSource::compute_queries() {
  ...
  auto const was_in_time =
      m_queries_present.wait_for(queue_lock, std::chrono::seconds(1),
  ...
  for (int i = 0; i < point_count; ++i) {
    ...
    // After the transformation the points are in NDC
    // Therefore check interval [-1, 1]
    if (std::abs(transformed_point.x()) > 1 &&
        std::abs(transformed_point.y() > 1))
      continue;

    points.push_back(point);
    colors.push_back(m_point_cloud.has_colors() ? m_point_cloud.clr(i)
                                                : rgb(1.f));
  }
...
PointCloudSource::get_finished_query() {
  if (m_pending_queries.empty()) return std::nullopt;
...
void PointCloudSource::queryPoints(
  ...
  if (!pending_queries_lock.owns_lock()) return;
